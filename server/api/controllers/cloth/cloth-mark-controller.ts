...
export class ClothMarkController extends BaseController {
    @Get('all')
    async getClothMarkAll(@Request() request: express.Request, @Query() user: string): Promise<ShoppingSuccessResponse<ClothMarkViewModel[]>> {
        console.log(user);
    ...
    @Post('info')
    async putClothMarkInfo(@Body() requestBody: ClothMarkRequest, @Request() request: express.Request): Promise<ShoppingSuccessResponse<ClothMarkRequest>> {
